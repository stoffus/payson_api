...
class PaymentTest < Test::Unit::TestCase
  ...
  def test_generated_hash_from_payment_data
    ...
    request.receivers.each_with_index do |receiver, index|
      email = request_hash[receiver_format % [index, 'email']]
      amount = request_hash[receiver_format % [index, 'amount']]
      first_name = request_hash[receiver_format % [index, 'firstName']]
      last_name = request_hash[receiver_format % [index, 'lastName']]
      primary = request_hash[receiver_format % [index, 'primary']]
    ...
    request.order_items.each_with_index do |order_item, index|
      description = request_hash[order_item_format % [index, 'description']]
      unit_price = request_hash[order_item_format % [index, 'unitPrice']]
      quantity = request_hash[order_item_format % [index, 'quantity']]
      tax = request_hash[order_item_format % [index, 'taxPercentage']]
      sku = request_hash[order_item_format % [index, 'sku']]
    ...
    request.fundings.each_with_index do |funding, index|
      constraint = request_hash[funding_format % [index, 'constraint']]
